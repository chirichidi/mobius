...
21. **익명 로그 라인의 귀속 규칙을 신호 종류마다 따로 적용해 절반만 지켜짐 (이슈 #19, 외부
    ...
    **며칠** 후보에서 빠졌다(hours → days). → 같은 PR에서 **모델 한도를 1급 개념으로 분리**했다:
    ...
    ★ **API가 아직 100%를 안 보여줘도 5시간 창이 95% 이상이면 버리지 않고 보류한다**
    (`nearLimitPercent`) — 로그 hit은 "CLI가 실제로 막혔다"는 사실이고 usage 백분율은
    근사값이라, 소진 순간 99%로 보이면 그대로 버려지고 막힌 사용자는 타이핑을 멈춰
    새 hit도 안 온다 = 영영 미기록. **주간 창에는 적용하지 않는다** — 주말 즈음이면 95%가
    정상 상태라, 그 상태에서 남의 hit이 영영 `.discard`에 못 닿아 15분 내내 조회가 도는
    상태로 굳는다(discard 카운터도 안 올라 백오프가 안 걸린다).
    ★ **쿨다운(120→180초)은 실측으로 정했다** — 제보자가 사고 로그에서 각 hit의 "그 요청이
    시작된 시각"까지 되짚어, **재시도 지연이 63초~2분 7초(127초)** 임을 측정했다(2026-08-22).
    즉 옛 120초는 **한 요청 분량도 못 덮어서** 전환 전에 시작된 턴의 에러가 쿨다운이 풀린 뒤
    도착했다. 시간 창으로 이 사고를 막으려던 원래 설계가 왜 실패했는지의 수치적 근거다.
    (지금은 쿨다운이 유일한 방어가 아니다 — 귀속은 usage 검증이 판정하고, 쿨다운은 그 위의
    연쇄 전환 방지다. `HitAttribution.modelScopeTrustWindow` 300초는 이미 127초 위였다.)
    ...
    **한 번도 새로 조회 못 한 채** 폴백으로 넘어갈 수 있다.
    ...
    ★ **신뢰 창 밖의 hit은 버린다(보류가 아니라)** — 모델 한도 100%는 며칠 그대로라
...
23. **계정 정체를 이메일 하나로 잡아 같은 이메일의 다른 조직이 프로필을 덮어씀 (외부 제보, 2026-09-11)** —
    ...
    **같은 프로필 하나가 덮어써져** 목록은 그대로고 직전 조직의 토큰은 `.bak`에만 남았다. 덤으로
    ...
    ★ 리뷰에서 드러난 경계 셋. (a) **대조 규칙은 한 함수로 모았어도 한 곳만 빠지면 그만이다** —
    `AppState.usageQueryBlob`이 관대한 `matches`를 직접 쓰고 있었고, 그러면 조직 미상 프로필이
    아직 활성 마커를 쥔 사이에 다른 조직 토큰으로 조회한 사용량이 그 프로필에 기록된다.
    폴백 경로(저장 스냅샷을 id로 꺼내기)는 애초에 오귀인 안전하므로 규칙을 조이면 폴백이 더 자주
    날 뿐 손해가 없다. (b) **모호하면 nil이다** — 조직을 모르는 열쇠에 같은 이메일 후보가 여럿인
    상태는 이 변경 **이후에** 처음 생긴다(그전엔 이메일당 하나였다). 배열 순서로 아무거나 잡으면
    남의 조직 토큰을 저장하고 setActive까지 간다. 21의 "잘못된 기록보다 미룬 판정"을 그대로 적용해
    `firstIndex`가 nil을 돌려주고, 소비자는 다음 틱에 다시 본다. (c) **UUID만 채우면 라벨과 토큰이
    어긋난 채 굳는다** — 구버전 되저장은 비밀만 덮어썼으므로 "라벨은 회사 Team, 저장 토큰은 개인
...
24. **토큰(Keychain)과 신원(~/.claude.json)을 같은 로그인이라고 가정하고 짝지어 저장함 (실사용, 2026-09-24)** —
    ...
    마킹하고, 다른 Claude 프로필의 저장본과 refresh 토큰이 같으면 refresh하지 않는다(누가 주인인지 모르므로
    마킹도 안 한다). 그래도 refresh 응답의 `organization.uuid`가 프로필 조직과 다르면(같은 종류의 두 조직)
    `.organizationMismatch`로 마킹하고, 회전본은 버리지 않고 그 조직의 같은 이메일 **비활성** 프로필이 정확히
    하나일 때 그쪽에 넘긴다. 서버가 이미 이전 토큰을 소비했으므로 버리면 살아남는 사본이 없다.
    ...
    reconcile은 보정하지 않는다 — 활성 마커를 옮기지 않는 것까지가 그 경로의 몫이다.
    남는 것: 보정이 안 되는 경우(좌석형 조직이 둘 이상이거나, 토큰이 활성이 아닌 프로필의 종류)는 여전히
    판정을 미루고, 그동안 동기화가 멈춘다. 등록하지 않은 같은 종류의 조직에서 온 토큰은 종류만으로 가를 수
    없어 활성 프로필에 붙을 수 있다 — 그 프로필이 비활성이 되어 refresh될 때 (b)의 응답 대조가 잡는다.

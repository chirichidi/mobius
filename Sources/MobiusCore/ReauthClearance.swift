...
public enum ReauthClearance {
    ...
    ///   다른 세션이 채운 토큰이 다른 조직의 것일 수는 있다. 종류가 다른 조직(좌석형 ↔ 개인 구독)이면 저장 전에
    ///   `liveSnapshotVerdict`가 거르지만, **같은 종류의 다른 조직**(Team A ↔ Team B)은 판정을 통과해 활성
    ///   프로필에 저장되고 여기서 딱지까지 내려간다 — 그 프로필이 비활성이 되어 refresh될 때 응답의 조직
    ///   대조가 잡는다(알려진 한계, 실패 기록 24 "남는 것"). 빈 토큰을 되저장했던 프로필이 CLI에서 다시 로그인해 멀쩡해진

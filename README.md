...
> [!IMPORTANT]
> **개인 계정은 1개 사용을 권장합니다.** 회사 ↔ 개인처럼 성격이 다른 계정을
> 오가는 것이 Mobius의 의도된 사용법입니다. 한도를 늘릴 목적으로 개인 계정을
> 2개 이상 돌려 쓰는 것은 각 서비스 약관에 어긋날 수 있으며, 이로 인해 계정이
> 제한·차단되더라도 Mobius는 책임지지 않습니다.
...
2. 로그인 창이 뜨면 추가할 Claude 계정으로 로그인 — 끝!
   (Codex는 터미널에서 `codex login` 한 번이면 몇 초 안에 자동 등록됩니다.)
...
- **같은 이메일로 여러 조직에 속한 경우** (예: 개인 Max + 회사 Team/Enterprise): 조직마다 별개
  계정으로 등록됩니다. "계정 추가"의 로그인 창에서 워크스페이스를 고르면 각각 카드가 생기고,
  구분이 쉽도록 두 번째부터는 닉네임에 조직 이름(개인 구독은 등급)이 붙습니다. 이전 버전에서
  등록한 계정은 첫 실행 때 저장된 스냅샷에서 조직 정보를 자동으로 채웁니다.
...
- **실행 중인 `claude` 세션**: 전환해도 기존 세션은 **끊기지 않고 계속 동작합니다**
  ...
  **세션을 새로 시작할 필요도 없습니다** — 이미 열려 있는 세션에 다음 입력을 넣는 순간부터
  새 계정으로 이어집니다. 이전 계정이 쓰이는 건 **전환 시점에 이미 진행 중이던 응답**뿐입니다
  (claude 2.1.232/2.1.233 기준). 혹시 전환 뒤에도 이전 계정 한도 에러가 계속 난다면
  그때는 세션을 새로 시작하면 확실히 해결됩니다.
  (2026-08-15 정정: 이전 문서는 "세션 재시작 필요"라고 적었지만, 그건 실측이 아니라
  "무중단" 실측에 붙었던 추측이었습니다.)
...
> 이 프로젝트는 오픈소스 개인 프로젝트이며 **Anthropic·OpenAI 어느 쪽과도 무관합니다.**
> "Claude"와 "Anthropic"은 Anthropic PBC의, "Codex"와 "ChatGPT"는 OpenAI의 상표이며,
> 여기서는 호환 대상 제품을 가리키기 위한 서술적 용도로만 사용됩니다.

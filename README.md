...
- **같은 이메일로 여러 조직에 속한 경우** (예: 개인 Max + 회사 Team/Enterprise): 조직마다 별개
  ...
  다만 **이전 버전에서 이미 덮어써진 조직은 되살아나지 않습니다** — 예전에는 같은 이메일이면
  프로필 하나를 공유했기 때문에, 마지막으로 로그인한 조직만 남아 있습니다. 자동으로 채우는 작업은
  남아 있는 프로필의 정보를 일관되게 맞출 뿐이므로, 사라진 조직은 "계정 추가"로 다시 등록하세요.
  v0.5.4에서 두 카드가 같은 사용량을 보이거나 전환할 때마다 재로그인을 요구했다면, 한 조직의
  토큰이 다른 조직 카드에 저장된 상태입니다. 이제는 토큰과 계정 정보가 서로 다른 조직을 가리키면
  저장하지 않고, 이미 섞인 카드는 토큰을 갱신할 때 "재로그인 필요"로 표시합니다. 그 카드의
  "다시 로그인"으로 해당 조직에 다시 로그인하면 복구됩니다.
